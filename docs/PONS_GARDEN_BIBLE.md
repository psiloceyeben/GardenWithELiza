...
- **I-6. PONS is never emitted as a gameplay reward.** Plants yield soft currency (Sap) only. Sap is not convertible, redeemable, bridgeable, or withdrawable to PONS or any token, ever. No marketplace where Sap-priced goods are resold for tokens.
  - **AMENDED 2026-09-08 by Benjamin, narrowly, for the season prize only.** A fixed seasonal prize of $100-equivalent in PONS may be paid to the top finishers of a free-entry skill competition, split by rank.
  - **The core of I-6 is unchanged and binding: there is no rate.** Sap remains non-convertible, non-redeemable, non-bridgeable and non-withdrawable. No player may exchange Sap, plants, endowments or vintages for PONS or any token at any published or negotiated rate. The prize is a fixed amount decided by rank, not a conversion; once paid the studio owes nobody anything.
  - **Prize sizing must be unambiguous.** The PONS quantity is fixed from a stated reference price at a stated moment (the closing bell), published in the Season Rules in advance. "$100 of PONS" with no fixed reference is a dispute waiting to happen.

- **SCOPE REDUCTION 2026-09-08 by Benjamin: the StonkBrokers layer is removed entirely.** The game does not read, count, display or reference broker NFTs or tokenized stock, and does not link to StonkBrokers. No US-player restriction is inherited, because no restricted asset is touched.
  - **Moot, retained only as history:** I-1, I-3, I-4 and I-5 in their StonkBrokers-specific application, plus `COUNSEL_PACKET_STOCK_REWARDS.md` and `PROPOSED_S0_AMENDMENT.md` in full.
  - **The chain surface is now one thing: a read-only PONS wallet scanner.** The game reads PONS balance history to derive land and to size in-game rewards. It signs nothing, holds nothing, moves nothing.
  - **The "stocks" are wholly fictional.** Plant names are a larp under the amended I-9. No plant represents, tracks or references a real security, price or feed, and nothing in the product touches a tokenized security.
